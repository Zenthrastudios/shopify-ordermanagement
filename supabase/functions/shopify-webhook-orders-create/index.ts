...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("Received order webhook:", { shopDomain, topic: shopifyTopic });
    ...
    if (!store) {
      console.log("Store not found, creating new store for:", shopDomain);
      ...
      await processOrder(supabase, orderData, newStore.id);
    } else {
...
async function processOrder(supabase: any, orderData: any, storeId: string) {
  ...
  console.log(`Order ${orderPayload.order_number} processed successfully`);
}
